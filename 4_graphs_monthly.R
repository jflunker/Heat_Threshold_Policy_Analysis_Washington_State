...
# This code is to make monthly graphs showing the total number of days exceeding HI thresholds (80,85,89,95) and maximum temperatures (80,85,89,95)
...
# to make monthly graphs showing the total number of days over HI thresholds (80,85,89,95) 
...
max_HI_month_time = cbind(max_HI_within_cells[,1:3], max_HI_row_means_80, max_HI_row_means_85, 
                          max_HI_row_means_89, max_HI_row_means_95)  

# to calculate the total number of days over XX at the monthly level

temp_part3={}
for (month_data in 12:1){
  temp_part1 = max_HI_month_time[which(max_HI_month_time$Month == month_data),]
  temp_part2 = cbind(month_data, sum(temp_part1$max_HI_row_means_80), sum(temp_part1$max_HI_row_means_85),
                     sum(temp_part1$max_HI_row_means_89), sum(temp_part1$max_HI_row_means_95))
  temp_part3 = rbind(temp_part2, temp_part3)
}

max_HI_month = data.frame(temp_part3)
colnames(max_HI_month) = c("month", "over80", "over85", "over89", "over95")

max_HI_month_m = melt(max_HI_month, id.vars = "month")

tiff("number_month_HI.tiff", width = 3000, height = 800,  res = 300, compression = 'lzw')

ggplot(max_HI_month_m, aes(x = month, y = value, fill = variable)) +
...
# to make monthly graphs showing the total number of days over maximum temperatures (80,85,89,95) 
# based on the average of cells within polygons
...
max_temp_month_time = cbind(extract_temp_all[,1:3], max_temp_row_means_80, max_temp_row_means_85, 
...
temp_part3={}
for (month_data in 12:1){
  temp_part1 = max_temp_month_time[which(max_temp_month_time$Month == month_data),]
  temp_part2 = cbind(month_data, sum(temp_part1$max_temp_row_means_80), sum(temp_part1$max_temp_row_means_85),
                     sum(temp_part1$max_temp_row_means_89), sum(temp_part1$max_temp_row_means_95))
  temp_part3 = rbind(temp_part2, temp_part3)
}

max_temp_month = data.frame(temp_part3)
colnames(max_temp_month) = c("month", "over80", "over85", "over89", "over95")

max_temp_month_m = melt(max_temp_month, id.vars = "month")

tiff("number_month_max_temperature.tiff", width = 3000, height = 800,  res = 300, compression = 'lzw')

ggplot(max_temp_month_m, aes(x = month, y = value, fill = variable)) +
  geom_bar(stat="identity", position = position_dodge()) +
  theme(legend.position="bottom") +
  scale_x_continuous(breaks = c(1:12),
                     labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", 
